...
import { deleteUserRecord, listAdminUsers } from "../services/portfolioService";
    ...
    <section className="space-y-5">
      ...
      <div className="overflow-hidden rounded-2xl border border-white/10">
        <table className="w-full text-left text-sm">
          <thead className="bg-white/5 text-zinc-400"><tr><th className="p-4">User</th><th className="p-4">Username</th><th className="p-4">Actions</th></tr></thead>
          <tbody>
            {filtered.map((user) => <tr className="border-t border-white/10" key={user.uid}><td className="p-4">{user.displayName || user.email}</td><td className="p-4">/{user.username || "unclaimed"}</td><td className="p-4"><Button variant="danger" onClick={() => remove(user.uid)}>Delete</Button></td></tr>)}
          </tbody>
        </table>
      </div>
    </section>
